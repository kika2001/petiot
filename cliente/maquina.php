   ...
   ?>
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
	  ...
	  // Se $gravar for true, vamos actualizar a base de dados <<<-------------------------------------------------------
			// Formulate Query
			$query = "UPDATE maquina SET intervalo='". $_POST["intervalo"]. "', quantidade='". $_POST["quantidade"]. "' WHERE idmaquina='". $maquinas[0]['idmaquina']. "'";
			// Perform Query
			$gravar = my_query($query);
	...
	//Verificar se esta vazio
	if (!$logs) {
		$log= "Não existe nenhuma registo";
		echo $log;
	} else {
		echo "<table border='1'>
		<tr>
		...
		for ($i=0; $i<21; $i++) {
			...
			if ($logs[$i]['action'] == 'boot'){
				echo "<td bgcolor='#FF0000'>" . $logs[$i]['action'] . "</td>";
			} else {
				echo "<td>" . $logs[$i]['action'] . "</td>";
			}
			echo "</tr>";
		}
		echo "</table>";
